...
for kfold_idx,(index_kf_train, index_kf_validation) in enumerate(kf.split(label_paths, lbls_for_split)):
    ...
    train_dataset = CustomDataset(tds, train_lbl,train_cls_lbl, True)
    train_loader = DataLoader(train_dataset, batch_size=CFG['BATCH_SIZE'], shuffle=True, num_workers=0)

    val_dataset = CustomDataset(vds, valid_lbl,valid_cls_lbl, False)
    ...
    train(model, optimizer, train_loader, val_loader, scheduler, device, CFG,outputpath,str(kfold_idx))
    break